...
| `acaclaw.env.list` | 列出 conda 环境，含大小、活跃状态、Python/R 版本 |
| `acaclaw.env.activate` | 切换活动环境 |
| `acaclaw.env.pip.list` | 列出活跃环境中的 Python 包（`pip list`） |
| `acaclaw.env.pip.install` | 通过 pip 安装 Python 包 |
| `acaclaw.env.pip.uninstall` | 卸载 Python 包 |
| `acaclaw.env.r.list` | 列出 R 包（来自 conda 环境或系统 R） |
| `acaclaw.env.r.install` | 将 R 安装到 conda 环境（`r-base`、`r-irkernel`、`r-essentials`） |
| `acaclaw.env.sys.list` | 列出 conda 环境中的系统工具 |
| `acaclaw.env.node.list` | 列出全局 Node.js 包 |
| `acaclaw.env.cuda.list` | 检测 GPU 硬件与 CUDA 软件栈 |
...
| **Environment** | Conda 环境、Python/R/Tools/CUDA/Node.js 包、安装 R、GPU 检测 | `acaclaw.env.list`、`acaclaw.env.pip.list`、`acaclaw.env.r.list`、`acaclaw.env.cuda.list`、`acaclaw.env.node.list` |
...
环境标签页提供活跃 conda 环境的详细视图，包含五个子标签页，分别展示不同的包类别。
...
| 字段 | 示例 | 来源 |
|---|---|---|
| 活跃环境 | `aca ● active` | `acaclaw.env.list` — 通过学科检测确定活跃环境 |
| Python 版本 | `Python 3.12.8` | `acaclaw.env.list` |
| R 版本 | `R 4.5.2` | `acaclaw.env.r.list` |
| 环境大小 | `1.8 GB` | `acaclaw.env.list` — 通过 `du -sk` 计算 |
| Active 徽章 | `Active`（绿色） | 当显示的环境为当前活跃环境时出现 |
...
**Python** — 列出活跃 conda 环境中的 pip 包（`pip list --format json`）。显示包名、版本和来源。

**Tools** — conda 环境中可用的系统工具（如 `conda`、`mamba`、`jupyter`、`git`、`curl`）。每个工具显示版本和简要说明。

**R** — 活跃环境中的 R 包。AcaClaw 从两个来源检测 R：

| 来源 | 显示方式 |
|---|---|
| 系统 R（`/usr/bin/R`） | 橙色横幅："Using **system R** (packages shared with the OS). [Install R into conda env]" |
| Conda R | 标准包列表，无横幅 |
| R 未安装 | 显示带「Install R into conda env」按钮的卡片 |

点击「Install R into conda env」会通过 `acaclaw.env.r.install` 网关方法运行 `conda install -n <env> -y -c conda-forge r-base r-irkernel r-essentials`（最长 10 分钟）。

**CUDA** — GPU 与 CUDA 工具链检测。显示硬件和软件组件：

| 检测步骤 | 检测内容 | 来源 |
|---|---|---|
| GPU 硬件 | GPU 型号（NVIDIA、Intel、AMD） | `lspci` |
| NVIDIA 驱动 | 驱动版本、CUDA 版本 | `nvidia-smi` |
| CUDA 工具链 | nvcc 版本 | `nvcc --version` |
| cuDNN | cuDNN 版本 | conda 环境中的 Python ctypes 探测 |
| PyTorch CUDA | PyTorch CUDA 支持 | `torch.cuda.is_available()` |
| TensorFlow GPU | TensorFlow GPU 设备 | `tf.config.list_physical_devices('GPU')` |

在没有 NVIDIA GPU 的系统上，标签页仍显示检测到的硬件（如 Intel 集成显卡）。

**Node.js** — 全局安装或环境中的 Node.js 包。显示通过 `npm ls -g --json` 获取的包（如 `openclaw`、`claude-code`、`npm`）。
...
| 安装 R 到 conda 环境 | 「Install R into conda env」按钮 | 插件方法 `acaclaw.env.r.install` — 后台运行 `conda install r-base r-irkernel r-essentials` |
| 添加学科软件包 | 环境标签页的 `Add Discipline` 按钮 | 插件方法 `acaclaw.env.install` — 创建/扩展带学科包的 conda 环境 |
| 搜索软件包 | 环境子标签页搜索框 | 按名称过滤已安装包 |
| 切换环境 | 环境下拉菜单 | 设置 AI 上下文的活动环境 |
| 查看 GPU/CUDA 信息 | CUDA 子标签页 | 插件方法 `acaclaw.env.cuda.list` — 检测 GPU 硬件与 CUDA 软件 |
...
### 未来：Electron 或 Tauri 外壳（Linux / Windows）

macOS 上，AcaClaw 已使用 `WKWebView`（系统 WebKit 引擎）提供原生 `.app` 包。编译后的 Swift 二进制约 60 KB，在程序坞显示 AcaClaw 图标，原生处理程序坞点击，关闭窗口即退出 — 运行时无需浏览器。

Linux 和 Windows 上，若需要系统托盘、原生通知或登录自启，可用桌面外壳在原生窗口加载同一 SPA。Lit 组件不变 — 外壳仅替代浏览器标签。

| 选项 | 优点 | 缺点 |
|---|---|---|
| **WKWebView (Swift)** | 约 60 KB、原生程序坞图标、无需浏览器 | 仅 macOS **（当前）** |
| **Electron** | 跨平台、系统托盘、原生通知 | 150+ MB 开销、捆绑 Chromium |
| **Tauri** | 约 5 MB 二进制、系统 WebView、Rust IPC | 生态较新、WebView 兼容性因系统而异 |

Linux 和 Windows 外壳首版未计划。基于浏览器的 SPA 已覆盖这些平台所需功能。
...
| 平台 | 安装包类型 | 状态 |
|---|---|---|
| macOS | 使用 WKWebView 的 `.app`（Swift 编译） | **当前** — 原生窗口、程序坞图标、无需浏览器 |
| macOS | `.dmg`（签名 + 公证） | 未来 |
| Windows | `.exe`（签名） | 未来 |
| Linux | `.AppImage` 或 `.deb`/`.rpm` | 未来 |
| 全平台 | Shell 脚本 + 浏览器向导 | **当前** — 所有平台的回退方案 |

---

## 桌面启动 {: #desktop-launch}

AcaClaw 作为本地 Web 应用运行：网关提供 UI，用户通过原生窗口或浏览器访问。macOS 上，`install-desktop.sh` 编译一个约 60 KB 的 Swift 二进制，打开原生 `WKWebView` 窗口 — 无需浏览器。其他平台上，`start.sh` 在默认浏览器中打开 UI。完整的认证流程、令牌生命周期与 WebSocket 握手说明见 [认证与启动](/zh-CN/auth-and-app-launch/)。
...
| **macOS** | `~/Applications/` 中的 `.app` 包 | 原生 WKWebView 窗口 | Swift 编译；程序坞显示 AcaClaw 图标，原生处理程序坞重复点击，无需浏览器。若 `swiftc` 不可用则回退为 `open URL` |
| **WSL2** | Windows 桌面上的 `.lnk` | Edge/Chrome `--app` 模式（直接指向浏览器） | 通过启动文件夹 VBS 在 WSL 内运行网关；快捷方式直接指向 Edge 以确保图标/固定稳定。浏览器配置存储在 `%LOCALAPPDATA%\AcaClaw\browser-app` |
