...
AcaClaw 自带浏览器 UI — 基于 Lit 的独立 SPA，面向学术工作流。OpenClaw 内置管理后台仍在单独路径下，供高级功能使用。

同一网关、同一端口、同一 WebSocket API。两个前端，服务不同人群。

```
http://localhost:2090/          → AcaClaw UI  （科研工作区）
http://localhost:2090/admin     → OpenClaw UI（通道、调试、cron — 不变）
```
...
OpenClaw 网关通过两种机制实现双 UI 共存：

1. **`gateway.controlUi.basePath`** — 将内置 UI 移到子路径（如 `/admin`）。默认 UI 只处理该前缀下的请求，根路径 `/` 空闲。

2. **插件 HTTP 路由** — 插件可通过 `registerHttpRoute()` 以 `match: "prefix"` 注册路由。在网关 HTTP 调度管线中，插件路由（优先级 10）在控制 UI SPA 回退（优先级 12）**之前**执行。
...
不 fork OpenClaw 的 UI 代码。不使用 `controlUi.root` 覆盖。OpenClaw 默认 UI 在 `/admin` 保持原样。

### 为何 AcaClaw 占用根路径

AcaClaw UI 在 `/`（根），OpenClaw 管理后台移到 `/admin`。若把 AcaClaw 放在 `/lab` 等子路径，虽可避免路由冲突，但 UX 更差：

| 因素 | AcaClaw 在 `/` | AcaClaw 在子路径 |
|---|---|---|
| 用户输入 `localhost:2090` | 看到 AcaClaw 工作区 | 看到 OpenClaw 管理后台（易困惑） |
| `/skills` 显示 | AcaClaw 技能浏览器 | OpenClaw 技能浏览器（受众不符） |
| 路由冲突 | 需排除 `/health*`、`/ready*`、`/api/*`、`/plugins/*` | 无 |
| 所需配置 | `basePath: "/admin"`（官方支持） | 无 |
| 目标用户 UX | 干净、直达 | 多学一段路径或需重定向 |

排除列表为 4 个前缀，在 OpenClaw 各版本中稳定（OpenClaw 自带 UI 也排除同一集合）。对科研用户的 UX 收益大于少量实现成本。

### URL 路由

两套 UI 均为带各自客户端路由的 SPA。网关按 URL 前缀返回正确的 `index.html` — 各 SPA 在内部处理子路径。

| URL | 由谁提供 | 视图 |
|---|---|---|
| `/` | AcaClaw SPA | 概览（健康、用量、快捷操作） |
| `/chat` | AcaClaw SPA | 对话界面 |
| `/usage` | AcaClaw SPA | 用量追踪 |
| `/skills` | AcaClaw SPA | 技能浏览器 |
| `/environment` | AcaClaw SPA | Conda 环境查看器 |
| `/backup` | AcaClaw SPA | 备份管理 |
| `/settings` | AcaClaw SPA | 设置 + `Advanced Admin →` 链接 |
| `/admin` | OpenClaw SPA | OpenClaw 概览 |
| `/admin/chat` | OpenClaw SPA | OpenClaw 对话 |
| `/admin/skills` | OpenClaw SPA | OpenClaw 技能浏览器 |
| `/admin/agents` | OpenClaw SPA | OpenClaw 智能体视图 |
| `/admin/channels` | OpenClaw SPA | 通道管理 |
| `/admin/debug` | OpenClaw SPA | 调试检查器 |
| `/api/*` | 网关 | REST API（保留，不由任一 SPA 提供） |
| `/health`、`/ready` | 网关 | 健康/就绪探测（由 AcaClaw 放行） |

`/skills` 与 `/admin/skills` 彼此独立 — 同名路径、不同 SPA，无冲突。
...
| **两套 UI、一个网关** | AcaClaw 自建清晰 UI；OpenClaw UI 在 `/admin` 不变；无需 fork；两类用户都照顾到。 |

### 各 UI 提供什么

| 功能 | AcaClaw UI（`/`） | OpenClaw UI（`/admin`） |
|---|---|---|
...
日常科研用 `/`。若需要通道或调试，设置页有链接：`Advanced Admin →`，打开 `/admin`。
...
AcaClaw 在 `/` 提供独立 UI。它不是 OpenClaw UI 的 fork — 而是共享同一网关 WebSocket API 的独立 SPA。OpenClaw 完整管理后台仍在 `/admin`，覆盖 AcaClaw UI 未包含的功能。
...
| **Settings** | 带预设的简化配置、审计日志、指向 `/admin` 的 `Advanced Admin →` 链接 | `config.get`、`config.set`、`acaclaw.audit.query` |
| **Setup wizard** | 首次启动引导（学科、API 密钥、工作区、安全） | `config.set`、`acaclaw.env.install` |

### `/admin` 可用功能（OpenClaw UI）

以下功能供有需要的用户通过 AcaClaw 设置中的 `Advanced Admin →` 使用：
...
| **Linux / Windows / 全平台** | AcaClaw UI：`http://localhost:2090/`，OpenClaw 管理：`http://localhost:2090/admin` |
...
AcaClaw 将自有 UI 构建为独立 SPA。AcaClaw 插件在 `/` 注册 HTTP 前缀路由以提供构建产物。OpenClaw 默认 UI 经 `gateway.controlUi.basePath` 移到 `/admin`。用户在任意浏览器访问 `http://localhost:2090` 使用 AcaClaw。

| 优势 | 说明 |
|---|---|
| 零额外安装 | 已内置 — 网关同时提供两套 UI |
| 跨平台 | macOS、Windows、Linux 行为一致 |
| 无需维护 fork | AcaClaw UI 独立 — OpenClaw 更新不必合并 |
| 保留 OpenClaw 功能 | `/admin` 完整管理后台 — 无功能缺失 |
...
| 因素 | 说明 |
|---|---|
| **无 fork、无合并** | AcaClaw UI 为独立代码库。OpenClaw UI 更新自动生效 — `npm install -g openclaw@latest` 更新 `/admin` 管理后台，不触碰 AcaClaw。 |
...
构建 AcaClaw 独立 UI 核心视图。配置 `basePath: "/admin"`，在 `/` 注册插件 HTTP 路由。
...
| 插件 HTTP 路由 | 在 `/` 提供 AcaClaw `dist/`，在 `/admin` 提供 OpenClaw UI |
| `Advanced Admin →` 链接 | 设置页链到 `/admin`（通道、调试、cron） |
