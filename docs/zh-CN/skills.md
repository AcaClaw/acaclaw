...
AcaClaw 将技能分为四个层级。每层用途不同，筛选标准也不同。

### 基础技能（随 OpenClaw 捆绑）

这些技能随 OpenClaw 本身发布。AcaClaw 直接继承 — 无需单独安装。

| 技能 | 作用 |
|---|---|
| `nano-pdf` | 读取并提取 PDF 文本 |
| `xurl` | 获取并解析网页内容 |
| `coding-agent` | 编写并执行代码 |
| `summarize` | 摘要文档与文本 |
...
建议每位研究者无论学科都安装。通过 Staff 面板从 ClawHub 安装。

| 技能 | 类别 | 作用 |
|---|---|---|
| `paper-search` | 文献 | 检索 arXiv、PubMed、Semantic Scholar |
| `academic-deep-research` | 文献 | 跨学术数据库开展透明、严谨的研究并保留审计轨迹 |
| `citation-manager` | 写作 | 按 APA、Vancouver、Nature 等格式排版参考文献 |
| `academic-writing` | 写作 | 学术论文、文献综述、方法学等方面的专家级智能体 |
| `ai-humanizer` | 写作 | 检测并弱化典型「AI 腔」写作模式 |
| `data-analyst` | 数据分析 | 数据可视化、报告、SQL、电子表格 |
| `chart-image` | 数据分析 | 为论文生成可发表质量的图表图像 |
| `mermaid` | 演示 | 从文本生成图表（流程图、时序图、类图等） |

**筛选规则**：同一用途只保留最佳工具。若两项技能功能重叠，只选更优者，不同时收录。

### 学科技能

面向特定研究领域的专项技能。当职员（staff）的学科匹配时，会在 Staff 面板中展示。

| 技能 | 学科 | 作用 |
|---|---|---|
| `bioskills` | 生物学 | 425 个生物信息学工具：RNA-seq、单细胞、变异检测、宏基因组等 |
| `lobster-bio-dev` | 生物学 | 面向协作基因组学流水线的多智能体生物信息学引擎 |
| `admet-prediction` | 生物学、化学、医学 | 药物/候选化合物的 ADMET 预测 |
| `chemistry-query` | 化学 | PubChem API：化合物信息、SMILES 结构、合成路线 |
| `paramus-chemistry` | 化学 | 大量化学与科学计算工具 |
| `clarity-research` | 化学、生物学 | 从 Clarity Protocol 检索蛋白质折叠研究数据 |
| `medical-research-toolkit` | 医学 | 查询 14+ 生物医学数据库，支持药物再利用与临床试验 |
| `medical-clinicaltrials` | 医学 | 使用高级方案筛选检索 ClinicalTrials.gov |
| `pmc-harvest` | 医学、生物学 | 从 PubMed Central 获取全文文章 |
| `pubmed-edirect` | 医学、生物学 | 通过 NCBI EDirect CLI 进行高级 PubMed 检索与获取 |
| `wolfram-alpha` | 物理学、数学 | 复杂计算、物理仿真、单位换算 |
| `acorn-prover` | 数学、物理学 | 使用 Acorn 定理证明器验证并撰写形式化证明 |
| `arxiv-cli-tools` | 物理学、数学、计算机科学 | 获取 arXiv 论文的 CLI 工具 |
| `agentic-paper-digest` | AI / ML | 自动获取并摘要近期 arXiv 与 Hugging Face 上的 AI/ML 论文 |
| `arxiv-paper-reviews` | AI / ML | 通过 arXiv Crawler 获取 AI/ML 论文并管理审稿笔记 |
| `github` | 计算机科学 | 操作 GitHub：议题、PR、CI 运行、高级查询 |
| `docker-essentials` | 计算机科学 | 容器管理必备的 Docker 命令 |
| `git-essentials` | 计算机科学 | 版本控制必备的 Git 命令 |
| `geepers-data` | 地球与环境 | 从 NASA、人口普查、气候 API、arXiv、PubMed 等获取数据 |
| `biodiversity-corridor-calculator` | 地球与环境 | 分析生物多样性廊道与生态连通性 |
| `autonomous-research` | 社会科学 | 面向定性或定量研究的多步自主研究 |
| `limesurvey` | 社会科学 | 自动化问卷创建与管理以采集数据 |
...
技能在 **OpenClaw 与 AcaClaw 之间共享**。两者使用相同的存储路径 — 没有单独的 AcaClaw 技能目录。
...
| 6（最高） | `<workspace>/skills/` | 工作区技能（例如 `~/AcaClaw/skills/`） |

### 对 AcaClaw 的含义

- **通过 `clawhub install` 安装的技能会进入 `~/.openclaw/skills/`**（托管目录）
- 原版 OpenClaw 与 AcaClaw 网关都能看到相同的托管技能
- 技能只需安装一次，各处可用
- AcaClaw 的 `OPENCLAW_HOME`（`~/.openclaw-acaclaw/`）控制 **配置与插件**，不控制技能路径
- 若仅为 AcaClaw 覆盖某项技能，请放在 `~/AcaClaw/skills/<skill>/`（工作区级覆盖）

### 为何要共享？

- **「扩展生态，避免分叉」** — 技能属于 OpenClaw 生态
- 不重复 — 安装一次，任意网关可用
- 在 AcaClaw 与原版 OpenClaw 之间切换无摩擦
- 如需，可用环境变量 `OPENCLAW_STATE_DIR` 覆盖默认的 `~/.openclaw`
...
官方 AcaClaw 技能位于 [github.com/acaclaw/acaclaw-skills](https://github.com/acaclaw/acaclaw-skills)。以下是团队的管理方式。
...
│   └── physics-tools/
├── tests/
│   ├── integration/           ← 跨技能兼容性测试
│   └── environment/           ← Conda 环境解析测试
└── scripts/
    ├── publish.sh             ← 发布到 ClawHub
    └── env-check.sh           ← 校验环境兼容性
```
...
|---|---|---|
| **单元测试** | 单项技能逻辑 | 每个 PR |
| **集成测试** | 在固定 OpenClaw 版本上技能能否正常运行 | 每个 PR |
| **环境测试** | 所有技能依赖在共享 Conda 环境中是否干净解析 | 每个 PR + 每夜 |
| **安全测试** | 无数据外泄、无危险命令、无凭据泄露 | 每个 PR |
| **兼容性测试** | 在标准与最高安全模式下均可工作 | 发布前 |
| **跨技能测试** | 技能之间互不干扰 | 发布前 |

### 维护工作流

1. **OpenClaw 发布新版本** → 针对新版本运行全部测试
2. **测试失败** → 维护者提交修复 PR；若上游破坏 API，向 OpenClaw 提 issue
3. **Conda 环境更新** → 重新解析全部依赖并运行环境测试
4. **报告缺陷** → 调试者调查，提交含修复与回归测试的 PR
5. **提议新技能** → 遵循 [贡献流程](#contributing-new-skills)

### 调试技能

当技能出现故障时：

1. **复现** — 本地用 `pnpm test -- <skill>.test.ts` 运行失败用例
2. **隔离** — 区分是技能、环境还是 OpenClaw 的问题；在干净 Conda 环境中重试
3. **查上游** — OpenClaw 是否变更 API？查阅变更日志
4. **修复与测试** — 提交 PR：修复 + 在修复前失败、修复后通过的回归测试
5. **发布** — PR 合并后由 CI 发布到 ClawHub

### 版本固定

主 acaclaw 仓库中的 `skills.json` 固定各 AcaClaw 发行版所附的技能版本：
...
这样每位 AcaClaw 用户获得的是经过测试的确切版本 — 而非 ClawHub 上的「最新任意版」。
