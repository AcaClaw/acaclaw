...
// AcaClaw patch: reverse map from accountId → sessionKey so the gateway's
// web.login.wait (which only passes accountId) can find the active login.
...
export async function startWeixinLoginWithQr(opts: {
  ...
  try {
    ...
    // AcaClaw patch: store accountId → sessionKey mapping for gateway lookup.
    // The gateway's web.login.wait only passes accountId, not sessionKey.
    accountToSession.set(opts.accountId ?? "", sessionKey);
...
export async function waitForWeixinLogin(opts: {
  ...
  // AcaClaw patch: resolve sessionKey from accountToSession map when the
  // gateway passes accountId (or empty string) instead of the real sessionKey.
  if (!activeLogin) {
    const resolved = accountToSession.get(opts.sessionKey);
    if (resolved) {
      activeLogin = activeLogins.get(resolved);
      if (activeLogin) {
        // Rewrite opts.sessionKey so downstream cleanup uses the real key
        opts = { ...opts, sessionKey: resolved };
      }
    }
  }
