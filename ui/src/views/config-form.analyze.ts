...
function normalizeSecretInputUnion(
  ...
  if (nonString.length !== 1 || !isSecretRefUnion(nonString[0])) return null;
  return normalizeSchemaNode(
    { ...schema, ...remaining[stringIndex], nullable, anyOf: undefined, oneOf: undefined, allOf: undefined },
    path,
  );
}
...
function normalizeUnion(
  ...
  if (literals.length > 0 && remaining.length === 0) {
    ...
    for (const value of literals) {
      if (!unique.some((existing) => Object.is(existing, value))) unique.push(value);
    }
    return {
      schema: { ...schema, enum: unique, nullable, anyOf: undefined, oneOf: undefined, allOf: undefined },
      unsupportedPaths: [],
    };
  }
  ...
  if (remaining.length === 1) {
    ...
    if (res.schema) res.schema.nullable = nullable || res.schema.nullable;
    return res;
  }
  ...
  if (
    ...
    return { schema: { ...schema, nullable }, unsupportedPaths: [] };
  }
