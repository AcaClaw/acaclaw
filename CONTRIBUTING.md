...
| **Environment compatibility** | Skill's deps resolve cleanly in the shared Conda env — no conflicts with other skills |
...
## Environment Compatibility (Purpose 3)

All skills share a single Conda environment (`env/conda/environment-base.yml`). When adding a new skill:

1. Declare only the packages your skill actually imports in the `requires` field of `skills.json`
